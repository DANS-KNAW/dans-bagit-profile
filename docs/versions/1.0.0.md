...
* **bag-ID**: a [urn:uuid]{:target=_blank} given to every deposit upon delivery to DANS. The bag-ID of the first version of a dataset is also known as the *sword token*.
...
* **dataset**: an information package containing data and metadata published in a *DANS Data Station*. 
...
* **deaccessioned version**: a dataset that is present in a *DANS Data Station*, but no longer disseminated. To the external user the
  dataset version appears to be deleted, only a tombstone remains.
...
* **sword token**: a [urn:uuid]{:target=_blank} given to first version DEPOSIT bags upon submission through the Sword Service. 
...
4. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. The value of this 
   ...
   in 1.2.3 MUST be specified to indicate that this deposit is an update of that dataset. 
...
1. The bag MUST have a tag-directory called `metadata` directly under the bag base
   directory.
...
2. If `bag-info.txt` contains the element `Is-Version-Of`, it MAY also contain an element On-Behalf-Of 
...
* [URIs of approved licenses]{:target=_blank} - licenses that may be used for datasets deposited in the DANS Data Stations
