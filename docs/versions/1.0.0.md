...
* Bag sequence requirements - requirements regarding the metadata that records how a sequence of bags can represent the
  history of a dataset <!-- TODO: REMOVE ? -->
...
7. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. The value of this 
   element MUST start with a client specific prefix agreed between the customer and DANS. If there already exists 
...
4. Any `dcx-dai:<scheme>` elements (i.e. DAI, ISNI, ORCID) MUST contain an identifier that complies with the syntaxis
