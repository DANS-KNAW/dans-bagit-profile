...
3. The `bag-info.txt` file MAY contain at most one element called `Is-Version-Of` with a
   [urn:uuid]{:target=_blank}-value.

4. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. The value of this 
...
#### 3.3 **(MIGRATION)** `metadata/depositor-info/agreements.xml`
...
2. If `bag-info.txt` contains the element `Is-Version-Of`, it MUST also contain an element Data-Station-User-Account 
