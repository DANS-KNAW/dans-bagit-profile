...
* Dataset update requirements - requirements regarding the metadata that records how subsequent versions of a dataset 
...
#### Stand-alone vs with-versions-context validation

There are two levels of validation:

* **stand-alone**: this includes the validation steps that can be performed without taking into account any data outside
  the bag, in particular possible previous versions of the deposit. This means that only the rules in sections 1-3
  are checked in this level.
* **with-versions-context**: this includes all the checks done in stand-alone validation plus checks that involve the
  dataset versions that the bag is (to be) part of. This means all the rules in this document are checked.
