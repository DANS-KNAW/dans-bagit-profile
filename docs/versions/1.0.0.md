...
* **version**: a *deposit* that has its own *version-bag-ID*. A *version* of an archived *dataset* may be updated by new
  *version*<hidden>s that keep the same *DOI*.

* **version-bag-ID**: a *bag-ID* that references a *version* of an archived *dataset*. If the *dataset* consists of several
  *versions*, the DOI resolves to the most recent of those *version*<hidden>s.

* **Sword Token**: a [urn:uuid] given to first version [DEPOSIT] bags upon submission through the Sword Service. 
  This UUID is used to identify all versions of one dataset. 
