...
2. (a) The `bag-info.txt` file MUST contain exactly one element called `Created`. (b) It MUST have a timestamp value in [ISO 8601]{:target=_blank} format,
...
4. Any `dcx-dai:<scheme>` elements (i.e. DAI, ISNI or ORCID) MUST contain an identifier that complies with the syntaxis for the selected identifier scheme URI.
...
1. The purpose of `original-filepaths.txt` is to provide a complete mapping from renamed files back to their original full path (including filename). It MUST be
   a text file encoded with UTF-8, with lines formatted as `<physical-bag-relative-path><whitespace><original-bag-relative-path>`, where
