...
what is acceptable as a MIGRATION bag. Where these are different, this is documented.
...
* **sword token**: a [urn:uuid]{:target=_blank} **(DEPOSIT)** given to first version bags upon submission through the
  Sword Service. This UUID is used when depositing an *update-deposit* and to identify all versions of one dataset.
...
4. (a) The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. The value of this
   ...
   dataset in the targeted Data Station with the same value in the field `dansOtherId` then Is-Version-Of as described
...
2. The `metadata` directory MUST contain the files: (a) `metadata/dataset.xml` and (b) `metadata/files.xml`. (c) **(
   MIGRATION)**
   It MAY contain the files `metadata/amd.xml`, `metadata/emd.xml`, `metadata/provenance.xml`
...
10. The file `metadata/dataset.xml` MUST have a rights holder, either **(DEPOSIT)** in `<dcterms:rightsHolder>` or **(
    MIGRATION)** as an author with role `RightsHolder`.
...
1. **(MIGRATION)** The file `metadata/depositor-info/agreements.xml` MUST adhere to [DANS deposit agreement metadata]{:
   target=_blank}.

2. **(MIGRATION)** The file `metadata/amd.xml` is REQUIRED and MUST adhere to [DANS administrative metadata]{:target=_
   blank}.
...
2. If `bag-info.txt` contains the element `Is-Version-Of`, it MAY also contain an element On-Behalf-Of with an existing
   Data Station user id. This account MUST have edit rights on the target dataset.
