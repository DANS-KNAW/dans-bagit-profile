...
2. The bag-info.txt file MAY contain one or more elements called Created, for backwards compatibility with previous versions of this specification. If present, 
   this element will be ignored. The client SHOULD indicate the correct order of the versions of a dataset by sending the corresponding deposits in that order.
