...
7. The `bag-info.txt` file MAY contain at most one element called `Has-Organizational-Identifier`. This value may be 
   used to identify duplicates and sequences. If this `Has-Organizational-Identifier` is given, an additional 
...
2. It MUST be formatted as `<physical-bag-relative-path><whitespace><orignal-bag-relative-path>`. 
     ...
     * `<original-bag-relative-path>` MUST correspond one-on-one to a `filepath` attribute in the `file` elements in the `files.xml`. 
...
4. Each `file` element MUST have a `filepath` attribute which records the correspondence with a payload file.

5. (a) The `filepath` attribute MUST contain the bag relative path of the payload file, (b) OR if `original-filepaths.txt` exists, rule 2.7.2 applies.
