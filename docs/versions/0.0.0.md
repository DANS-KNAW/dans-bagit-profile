...
5. (a) The `filepath` attribute MUST contain the bag relative path of the payload file and every payload file MUST be represented exactly once in the files.xml, (b) OR if `original-filepaths.txt` exists, rule 2.7.2 applies.
